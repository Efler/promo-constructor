import { useState } from 'react'
import {
  Alert,
  Badge,
  Box,
  Button,
  Card,
  Group,
  Image,
  Paper,
  SimpleGrid,
  Skeleton,
  Stack,
  Text,
  ThemeIcon,
  Title,
} from '@mantine/core'
import { useQuery } from '@tanstack/react-query'
import { IconAlertCircle, IconBoxSeam, IconChevronDown, IconPackage, IconSparkles } from '@tabler/icons-react'
import { apiRequest } from '../../shared/api/client'

type BundleProductCard = {
  id: number
  title: string
  brand: string | null
  description: string | null
  subject_name: string | null
  parent_name: string | null
  main_photo_url: string | null
  is_active: boolean
  item_count: number
  total_stock_qty: number
  min_price: string
  min_discounted_price: string | null
  max_discount_percent: number
  sizes: string[]
}

function formatMoney(value: string | null) {
  if (!value) {
    return null
  }

  return new Intl.NumberFormat('ru-RU', {
    style: 'currency',
    currency: 'RUB',
    maximumFractionDigits: 0,
  }).format(Number(value))
}

export function BundlesWorkspacePage() {
  const [isVisible, setIsVisible] = useState(false)

  const productsQuery = useQuery({
    queryKey: ['bundle-products'],
    queryFn: () => apiRequest<BundleProductCard[]>('/products/bundle-cards'),
    enabled: false,
  })

  async function handleRevealProducts() {
    setIsVisible(true)
    await productsQuery.refetch()
  }

  const products = productsQuery.data ?? []

  return (
    <Stack gap="xl">
      <Paper
        p={{ base: 'lg', md: 'xl' }}
        radius="xl"
        shadow="md"
        style={{
          border: '1px solid rgba(113, 33, 93, 0.08)',
          background:
            'linear-gradient(135deg, rgba(255,255,255,0.96), rgba(248,241,250,0.92))',
        }}
      >
        <Group justify="space-between" align="start" gap="lg">
          <Box maw={760}>
            <Badge variant="light" color="brand" mb="md">
              Beta
            </Badge>
            <Title order={2} c="dark.8">
              Комплекты
            </Title>
            <Text c="dimmed" mt="sm" size="lg">
              Раздел подготовлен для будущей механики комплектов. Пока здесь можно
              быстро посмотреть ассортимент текущего продавца и выбрать товары-кандидаты.
            </Text>
          </Box>

          <ThemeIcon size={56} radius="xl" color="brand">
            <IconPackage size={28} />
          </ThemeIcon>
        </Group>
      </Paper>

      <SimpleGrid cols={{ base: 1, lg: 2 }} spacing="xl" style={{ alignItems: 'start' }}>
        <Box>
          <Paper
            p={{ base: 'lg', md: 'xl' }}
            radius="xl"
            shadow="sm"
            style={{ border: '1px solid rgba(113, 33, 93, 0.08)' }}
          >
            <Stack gap="lg">
              <div>
                <Title order={4}>Быстрый просмотр ассортимента</Title>
                <Text c="dimmed" mt="sm">
                  Загрузи список товаров продавца, чтобы оценить, из чего дальше
                  собирать комплекты.
                </Text>
              </div>

              <SimpleGrid cols={1} spacing="sm">
                <Paper radius="lg" p="md" bg="brand.0">
                  <Text fw={700}>Что покажем</Text>
                  <Text size="sm" c="dimmed" mt={6}>
                    Название, бренд, категория, размеры, остатки и актуальные цены.
                  </Text>
                </Paper>
                <Paper radius="lg" p="md" bg="brand.0">
                  <Text fw={700}>Что не показываем</Text>
                  <Text size="sm" c="dimmed" mt={6}>
                    Технические идентификаторы и служебные поля backend-части.
                  </Text>
                </Paper>
              </SimpleGrid>

              <Button
                size="md"
                color="brand"
                leftSection={<IconBoxSeam size={18} />}
                rightSection={<IconChevronDown size={16} />}
                loading={productsQuery.isFetching}
                onClick={handleRevealProducts}
              >
                Показать товары продавца
              </Button>
            </Stack>
          </Paper>
        </Box>

        <Box>
          {!isVisible ? (
            <Paper
              p="xl"
              radius="xl"
              shadow="sm"
              style={{
                border: '1px dashed rgba(188, 61, 150, 0.4)',
                background:
                  'linear-gradient(180deg, rgba(252,248,253,1) 0%, rgba(255,255,255,1) 100%)',
              }}
            >
              <Stack align="center" gap="sm" py="xl">
                <ThemeIcon size={54} radius="xl" variant="light" color="brand">
                  <IconSparkles size={26} />
                </ThemeIcon>
                <Title order={4}>Ассортимент пока скрыт</Title>
                <Text c="dimmed" ta="center" maw={520}>
                  Нажми на кнопку слева, и мы подгрузим красивый обзор товаров текущего seller.
                </Text>
              </Stack>
            </Paper>
          ) : null}

          {productsQuery.isFetching ? (
            <Stack gap="md">
              {[1, 2, 3].map((item) => (
                <Paper key={item} p="lg" radius="xl" shadow="sm" style={{ border: '1px solid rgba(113, 33, 93, 0.08)' }}>
                  <Stack gap="md">
                    <Skeleton height={28} radius="md" />
                    <Skeleton height={16} width="70%" radius="md" />
                    <Skeleton height={84} radius="lg" />
                  </Stack>
                </Paper>
              ))}
            </Stack>
          ) : null}

          {!productsQuery.isFetching && productsQuery.isError ? (
            <Alert
              radius="xl"
              color="red"
              variant="light"
              icon={<IconAlertCircle size={18} />}
              title="Не удалось загрузить товары"
            >
              Проверь доступность backend API и наличие активной seller-сессии.
            </Alert>
          ) : null}

          {!productsQuery.isFetching && !productsQuery.isError && isVisible && products.length === 0 ? (
            <Paper
              p="xl"
              radius="xl"
              shadow="sm"
              style={{ border: '1px solid rgba(113, 33, 93, 0.08)' }}
            >
              <Stack align="center" gap="sm" py="lg">
                <Title order={4}>Товары не найдены</Title>
                <Text c="dimmed" ta="center">
                  У текущего seller пока нет активных товарных позиций для обзора.
                </Text>
              </Stack>
            </Paper>
          ) : null}

          {!productsQuery.isFetching && !productsQuery.isError && products.length > 0 ? (
            <Stack gap="md">
              {products.map((product) => (
                <Card
                  key={product.id}
                  padding="lg"
                  radius="xl"
                  shadow="sm"
                  style={{ border: '1px solid rgba(113, 33, 93, 0.08)' }}
                >
                  <Group align="start" wrap="nowrap" gap="lg">
                    <Image
                      src={product.main_photo_url ?? 'https://placehold.co/220x160/f7edf6/7e3a68?text=No+Photo'}
                      alt={product.title}
                      radius="lg"
                      w={220}
                      h={160}
                      fit="cover"
                    />

                    <Stack gap="md" style={{ flex: 1 }}>
                      <Group justify="space-between" align="start" gap="md">
                        <div>
                          <Title order={4}>{product.title}</Title>
                          <Group gap="xs" mt={8}>
                            {product.brand ? <Badge variant="light">{product.brand}</Badge> : null}
                            {product.subject_name ? <Badge variant="outline" color="gray">{product.subject_name}</Badge> : null}
                            {product.parent_name ? <Badge variant="outline" color="brand">{product.parent_name}</Badge> : null}
                          </Group>
                        </div>

                        <Badge color={product.is_active ? 'green' : 'gray'} variant="light">
                          {product.is_active ? 'Активен' : 'Скрыт'}
                        </Badge>
                      </Group>

                      <Text c="dimmed" lineClamp={2}>
                        {product.description ?? 'Описание для этого товара пока не заполнено.'}
                      </Text>

                      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="sm">
                        <Paper radius="lg" p="sm" bg="gray.0">
                          <Text size="xs" c="dimmed" tt="uppercase" fw={700}>
                            От цены
                          </Text>
                          <Text fw={700} mt={4}>
                            {formatMoney(product.min_discounted_price ?? product.min_price)}
                          </Text>
                          {product.min_discounted_price ? (
                            <Text size="sm" c="dimmed">
                              Базовая: {formatMoney(product.min_price)}
                            </Text>
                          ) : null}
                        </Paper>

                        <Paper radius="lg" p="sm" bg="gray.0">
                          <Text size="xs" c="dimmed" tt="uppercase" fw={700}>
                            Остаток
                          </Text>
                          <Text fw={700} mt={4}>
                            {product.total_stock_qty} шт.
                          </Text>
                          <Text size="sm" c="dimmed">
                            Позиций: {product.item_count}
                          </Text>
                        </Paper>

                        <Paper radius="lg" p="sm" bg="gray.0">
                          <Text size="xs" c="dimmed" tt="uppercase" fw={700}>
                            Макс. скидка
                          </Text>
                          <Text fw={700} mt={4}>
                            {product.max_discount_percent}%
                          </Text>
                          <Text size="sm" c="dimmed">
                            Среди доступных размеров
                          </Text>
                        </Paper>
                      </SimpleGrid>

                      <div>
                        <Text size="sm" fw={600} mb={8}>
                          Размеры и варианты
                        </Text>
                        <Group gap="xs">
                          {product.sizes.map((size) => (
                            <Badge key={`${product.id}-${size}`} radius="sm" variant="light" color="brand">
                              {size}
                            </Badge>
                          ))}
                        </Group>
                      </div>
                    </Stack>
                  </Group>
                </Card>
              ))}
            </Stack>
          ) : null}
        </Box>
      </SimpleGrid>
    </Stack>
  )
}
